...
const foodMeetups: FoodMeetup[] = [
  {
    id: 'samgyeopsal-hongdae',
    regionId: 'hongdae',
    storeName: '松亭三代猪五花',
    location: '弘大入口站',
    distanceText: '步行 6 分钟',
    tags: ['韩式烤肉', '本地人常去', '人气排队店'],
    currentPeople: 2,
    maxPeople: 4,
    time: '今天 19:30',
    imageTone: 'bbq',
  ...
  {
    id: 'tteokbokki-yeonnam',
    regionId: 'hongdae',
    storeName: '延南洞辣炒年糕',
    location: '弘大入口站',
    distanceText: '步行 9 分钟',
    tags: ['辣炒年糕', '小吃', '学生最爱'],
    currentPeople: 1,
    maxPeople: 3,
    time: '今天 18:00',
...
const avatarGradients = [
  'linear-gradient(135deg,#48F0B4,#0A6B53)',
  'linear-gradient(135deg,#F2B84B,#48F0B4)',
  'linear-gradient(135deg,#D8FFF2,#16483B)',
  'linear-gradient(135deg,#A6FFE1,#1A2E29)',
];
    ...
    <main className="theme-food-meetups tan-mobile-frame relative h-dvh overflow-hidden bg-[#050806] text-white">
            ...
            {categoryTabs.map((tab) => {
              ...
              return (
                <div key={tab} className="relative flex h-11 flex-col items-center justify-start">
                  ...
                  >
                    <span>{tab}</span>
                    {active && <span className="mt-1 h-[3px] w-10 rounded-full bg-[linear-gradient(90deg,#8FF3D0,#5FE0B5,#48D6A0)] shadow-[0_0_8px_rgba(95,224,181,0.35)]" />}
                  </button>
                  {tab !== '地图' && (
                    <House
                      size={14}
                      fill="currentColor"
                      className={`absolute top-7 ${active ? 'text-[#48F0B4] drop-shadow-[0_0_6px_rgba(95,224,181,0.42)]' : 'text-white/58'}`}
                    />
                  )}
                </div>
              );
      ...
      <section className="relative z-10 h-full overflow-y-auto px-4 pb-[calc(92px+env(safe-area-inset-bottom))] pt-[calc(136px+env(safe-area-inset-top))] tan-scrollbar-hide">
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            <h2 className="text-[34px] font-extrabold leading-tight tracking-normal text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.48)]">
              找饭搭子
            </h2>
        ...
        <section className="rounded-[22px] border border-white/10 bg-[rgba(8,12,10,0.58)] p-3 shadow-[0_10px_28px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-[18px]">
...
function FoodMeetupCard({
  ...
  return (
    <article className="grid min-h-[118px] grid-cols-[98px_1fr_92px] gap-2 rounded-[18px] border border-white/10 bg-[rgba(8,12,10,0.56)] p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-[14px]">
      <div className={`food-thumb food-thumb-${meetup.imageTone}`}>
        <span>{meetup.imageTone === 'bbq' ? '烤肉' : meetup.imageTone === 'cafe' ? 'CAFE' : '小吃'}</span>
      </div>
      <div className="min-w-0 py-1">
        ...
        <button
          type="button"
          onClick={onStoreLink}
          className="mt-2 flex h-8 max-w-[116px] items-center justify-center gap-1.5 rounded-full border border-white/12 bg-black/18 px-3 text-[12px] font-semibold text-white/78"
        >
          <Link2 size={14} />
          店铺链接
      ...
      <div className="flex flex-col items-end justify-between py-1">
        ...
        <div className="flex -space-x-2">
          {Array.from({ length: Math.min(meetup.currentPeople, 4) }).map((_, index) => (
            <span
              key={index}
        ...
        <button
          type="button"
          onClick={onJoin}
          className="h-9 w-[86px] rounded-full bg-[linear-gradient(135deg,#8FF3D0_0%,#5FE0B5_48%,#48D6A0_100%)] text-[14px] font-extrabold text-[#073B32]"
        >
          一起去
        </button>
...
function FoodLockedSlot() {
  return (
    <div className="grid h-[64px] grid-cols-[74px_1fr_104px] items-center gap-2 rounded-[16px] border border-white/10 bg-[rgba(14,16,16,0.54)] px-2 opacity-68">
